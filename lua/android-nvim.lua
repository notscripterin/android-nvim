...
local log = require("android-nvim.log")
...
local function build_release()
    ...
    if gradlew == nil then
        log.error("Build failed: gradlew is not found.")
        return
    end
    ...
    timer:start(
        ...
        vim.schedule_wrap(function()
            ...
            log.info("Building release for " .. time_passed .. " seconds.")
        end)
    )
    ...
    vim.system(
        ...
        vim.schedule_wrap(function(obj)
            ...
            if obj.code == 0 then
                ...
                log.error("Build failed: " .. obj.stderr)
            end
        end)
    )
end
...
local function build_and_install(root_dir, gradle, adb, device)
    ...
    timer:start(
        ...
        vim.schedule_wrap(function()
            time_passed = time_passed + 1
            log.info("Building for " .. time_passed .. " seconds.")
        end)
    )
    ...
    vim.system(
        ...
        vim.schedule_wrap(function(obj)
            ...
            local install_obj = vim.system(
                { adb, "-s", device.id, "install", root_dir .. "/app/build/outputs/apk/debug/app-debug.apk" },
                {}
            ):wait()
            if install_obj.code ~= 0 then
                log.error("Installation failed: " .. install_obj.stderr)
                return
            end
            ...
            if application_id == nil then
                log.error("Failed to launch application, did not find application id")
                return
            end

            local main_activity = find_main_activity(adb, device.id, application_id)
            if main_activity == nil then
                log.error("Failed to launch application, did not find main activity")
                return
            end
            ...
            log.info("Successfully built and launched the application!")
...
local function build_and_run()
    ...
    if android_sdk == nil or #android_sdk == 0 then
        log.error("Android SDK is not defined.")
        return
    end
    ...
    if #running_devices == 0 then
        log.warn("Build failed: no devices are running.")
        return
    end
    ...
    vim.ui.select(running_devices, {
        ...
        if choice then
            log.info("Device selected: " .. choice.name)
...
local function uninstall()
    local gradlew = find_gradlew()
    if gradlew == nil then
        log.error("Uninstall failed: gradlew is not found.")
        return
    end

    local application_id = find_application_id(gradlew.cwd)
    if gradlew == nil then
        log.error("Uninstall failed: could not find application id.")
        return
    end
    ...
    local running_devices = get_running_devices(adb)
    if #running_devices == 0 then
        log.warn("Uninstall failed: no devices are running.")
        return
    end
    ...
    vim.ui.select(running_devices, {
        ...
        if choice then
            ...
            if uninstall_obj.code == 0 then
                ...
                log.error("Uninstall failed: " .. uninstall_obj.stderr)
            end
        else
...
local function launch_avd()
    ...
    if avds_obj.code ~= 0 then
        log.warn("Cannot read emulators")
        return
    end
    ...
    vim.ui.select(avds, {
        ...
        if choice then
            log.info("Device selected: " .. choice .. ". Launching!")
            vim.system(
                { emulator, "@" .. choice },
                { text = true },
                vim.schedule_wrap(function(obj)
                    if obj.code ~= 0 then
                        log.warn("Launch failed: " .. obj.stderr)
                    end
                end)
            )
        else
            log.warn("Launch cancelled.")
        end
    end)
end
...
local function refresh_dependencies()
    local gradlew = find_gradlew()
    if gradlew == nil then
        log.error("Refreshing dependencies failed, not able to find gradlew")
        return
    end
    ...
    vim.system(
        ...
        vim.schedule_wrap(function(obj)
            if obj.code ~= 0 then
                log.error("Refreshing dependencies failed: " .. obj.stderr)
                return
            end
            log.info("Refreshing dependencies sucessfully")
        end)
    )
end
...
local function get_templates()
    ...
    local template_sources_dir = {}

    local templates_dir, templates_dir_kmp
    local android_templates = vim.g.android_templates
    local android_templates_dir = vim.g.android_templates_dir
    local kmp_templates = vim.g.kmp_templates
    local kmp_templates_dir = vim.g.kmp_templates_dir

    for _, path in ipairs(vim.api.nvim_list_runtime_paths()) do
        if path:match("android%-nvim") then
            templates_dir = path .. "/templates/android"
            templates_dir_kmp = path .. "/templates/kmp"
            break
        end
    end

    if templates_dir then
        table.insert(template_sources_dir, templates_dir)
    end
    if templates_dir_kmp then
        table.insert(template_sources_dir, templates_dir_kmp)
    end
    if android_templates_dir then
        table.insert(template_sources_dir, android_templates_dir)
    end
    if kmp_templates_dir then
        table.insert(template_sources_dir, kmp_templates_dir)
    end

    local function extend_template_sources(dir)
        if vim.fn.isdirectory(dir) then
            vim.list_extend(template_sources, vim.fn.globpath(dir, "*", 0, 1))
        end
    end

    for _, dir in ipairs(template_sources_dir) do
        extend_template_sources(dir)
    end

    if android_templates then
        vim.list_extend(template_sources, android_templates)
    end
    if kmp_templates then
        vim.list_extend(template_sources, kmp_templates)
    end
...
local function is_kmp_template(path)
    local shared_path = path .. "/shared/src/"
    local kmp_dirs = {
        "commonMain",
        "androidMain",
        "iosMain",
    ...
    for _, dir in ipairs(kmp_dirs) do
        if vim.fn.isdirectory(shared_path .. dir) == 1 then
            return true
        end
    end
end

local function remove_empty_parents(path, stop_at)
    while path ~= stop_at and vim.fn.isdirectory(path) == 1 do
...
local function move_package(package_root, from_package_path, to_package_path)
    local src_path = package_root .. "/app/src/"
    local variants = vim.fn.readdir(src_path, function(name)
        return vim.fn.isdirectory(src_path .. "/" .. name) == 1
    end)
    for _, variant in ipairs(variants) do
        local java_path = src_path .. variant .. "/java/"
        local from = java_path .. from_package_path
        vim.fn.mkdir(java_path .. to_package_path, "p")
        vim.fn.rename(from, java_path .. to_package_path)
        remove_empty_parents(vim.fn.fnamemodify(from, ":h"), java_path)
    end
end

local function update_template(name, package, template, package_root)
    ...
    local settings_file = package_root .. "/settings.gradle.kts"
    local gradle_file = package_root .. "/app/build.gradle.kts"
    local manifest_file = package_root .. "/app/src/main/AndroidManifest.xml"
    ...
    move_package(package_root, template_package_path, package_path)
...
local function create_compose_from_template(name, package, template, project_root)
    ...
    if is_kmp_template(template.path) then
        update_template_kmp()
    else
        update_template(name, package, template, project_root)

        local main_activity_path = project_root
            .. "/app/src/main/java/"
            .. package:gsub("%.", "/")
            .. "/MainActivity.kt"
        ...
        if main_activity_path then
            ...
            log.warn("MainActivity.kt not found.")
        end
    end
end
...
local function create_new_compose()
    ...
    async.void(function()
        ...
        log.info("Project created at " .. project_root)
    end)()
end
